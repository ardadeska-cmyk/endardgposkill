...
local function createTab(name)
    ...
    local page = Instance.new("ScrollingFrame", Container); page.Size = UDim2.new(1, 0, 1, 0); page.BackgroundTransparency = 1; page.Visible = false; page.ScrollBarThickness = 2; page.CanvasSize = UDim2.new(0,0,2.5,0)
...
--// REFINED FISHING LOGIC (STRICT REMOTE USAGE)
    ...
    while getgenv()[isEnabledVar] and _G.HubActive do
        pcall(function()
            ...
            if rod and hum and hrp then
                ...
                -- Throw Remote (Senin verdiğin Args)
                local throwArgs = {{
                ...
                -- Bekleme (21s veya Slider)
                task.wait(waitTime)
                
                -- Reel Remote (Senin verdiğin Args)
...
-- AUTO FISH NEW (SABİT 21S)
createToggle(FarmPage, "Auto Fish [New]", false, function(v)
    getgenv().AutoFishNewEnabled = v
    if v then task.spawn(function() runFishing(21, "AutoFishNewEnabled") end) end
end)
...
createToggle(FarmPage, "Auto Fish (Logic)", false, function(v)
    ...
    if v then task.spawn(function() 
        while getgenv().AutoFishEnabled and _G.HubActive do
            runFishing(getgenv().FishTime, "AutoFishEnabled")
            task.wait(0.1)
        end
    end) end
end)

-- AUTO SELL FISH (TÜM LİSTE)
    ...
    if v then task.spawn(function() 
        ...
        while getgenv().FishSellEnabled and _G.HubActive do
            for _, fName in pairs(FishToSell) do
                pcall(function()
                    local sellArgs = {[1] = {["Fish"] = fName, ["All"] = true, ["Method"] = "SellFish"}}
                    ShopRemote:InvokeServer(unpack(sellArgs))
                end)
                task.wait(0.05)
            end
            task.wait(1)
...
-- AUTO BUY COMMON BAIT (200'ÜN ALTI KONTROLÜ)
createToggle(FarmPage, "Auto Buy Common Bait", false, function(v)
    ...
    if v then task.spawn(function() 
        while getgenv().AutoBuyEnabled and _G.HubActive do
            pcall(function() 
                local gui = LocalPlayer.PlayerGui:FindFirstChild("FishingBaitGui")
                if gui then 
                    local count = tonumber(gui.Main.List["Common Fish Bait"].ItemCount.Text:match("%d+"))
                    if count and count < 200 then 
                        local buyArgs = {
                            [1] = workspace.BuyableItems:FindFirstChild("Common Fish Bait"),
                            [2] = 99
                        }
                        ReplicatedStorage.Events.Shop:InvokeServer(unpack(buyArgs)) 
                    end 
                end 
            end) 
            task.wait(60)
...
--// MERCHANT, VISUALS, SETTINGS (BOZULMADI)
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.Rainbow then 
        ...
        MainStroke.Color = c; Title.TextColor3 = c
    end
end)
